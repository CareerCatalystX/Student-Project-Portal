...
export default function GetStartedButton({
  ...
  return (
    <div className="min-h-12 w-56">
      <button
        className={cn(
          "group flex h-11 w-full items-center justify-center gap-3 rounded-full bg-violet-200 p-2 font-lg transition-colors duration-100 ease-in-out hover:bg-purple-800",
          className,
        )}
      >
        <span
          className={cn(
            "text-purple-800 transition-colors duration-100 ease-in-out group-hover:text-violet-200",
          )}
        >
          {text}
        </span>
        <div
          className={cn(
            ...
            "bg-purple-800 group-hover:bg-violet-200",
          )}
        >
          <div className="absolute left-0 flex h-7 w-14 -translate-x-1/2 items-center justify-center transition-all duration-200 ease-in-out group-hover:translate-x-0">
            ...
            <ArrowRight
              size={16}
              className={cn(
                "size-7 transform p-1 text-violet-200 opacity-100 transition-transform duration-300 ease-in-out group-hover:opacity-0",
              )}
            />
          </div>
        </div>
