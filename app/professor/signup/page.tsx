  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        router.push(`/professor/verify-otp?email=${encodeURIComponent(values.email)}`); // Redirect to OTP verification page
      } else {
        const data = await response.json();
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-50 px-4 py-12">
      <Card className="w-full max-w-2xl">
        <CardHeader className="space-y-1">
          ...
          <CardDescription>
            Enter your information to create your professor account
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
